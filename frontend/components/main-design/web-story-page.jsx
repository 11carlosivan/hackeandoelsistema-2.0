...
export default function WebStoryPage({ story }) {
  const html = storyHtml(story);
  const safeHtml = html ? sanitizeEditorialHtml(html) : null;
  ...
  return (
    <div className="w-full bg-background text-on-surface">
      <SystemPageHeader
        eyebrow="WEB STORY"
        title={story.title}
        description="Historia visual publicada en Hackeando el Sistema."
        stats={[
        ...
        <article className="border border-terminal-gray bg-surface-container-low/25 p-6 md:p-8">
          {safeHtml ? (
            ...
            <p className="text-on-surface-variant leading-relaxed">
              Esta Web Story esta lista para una experiencia visual dedicada.
            </p>
          )}
        </article>
      </section>
