...
describe('resolveRoute', () => {
  ...
  it('resolves every public category navigation route', () => {
    const categoryLinks = siteConfig.mainNav.filter((item) => item.href.startsWith('/category/'));

    expect(categoryLinks.length).toBeGreaterThan(5);
    categoryLinks.forEach((item) => {
      const route = resolveRoute(item.href);

      expect(route.found).toBe(true);
      expect(route.entityType).toBe('CATEGORY');
      expect(route.httpStatus).toBe(200);
      expect(route.seo.title).toContain('Hackeando el Sistema');
    });
  });
